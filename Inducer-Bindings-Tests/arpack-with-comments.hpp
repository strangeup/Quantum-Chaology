...
extern "C" //dlarnv written in C
{
  void dlarnv_(int *idist, int *iseed, int *n, double *x);
  //D: returns vector of n random real numbers from normal or uniform
}

namespace boost { namespace numeric { namespace bindings {  namespace arpack {

  enum which_eigenvalues { LARGEST_MAGNITUDE, //D: defines type consisting of this list
    SMALLEST_MAGNITUDE,
    LARGEST_REAL_PART,
    SMALLEST_REAL_PART,
  ...
  namespace detail
  {
    const char *map_which_to_string(which_eigenvalues we)
    {
      switch (we)
      {
        case LARGEST_MAGNITUDE: //D: map enum to string 
          return "LM";
  ...
  enum arpack_mode { //D: another definition 
  ...
  struct results //D: struct to contain results
  {
      ...
      >::real_type real_type; //D: real_type definition
      typedef std::complex<real_type> complex_type; //D: complex version of real

      typedef std::vector<complex_type> value_container; //D: vector of complex type
      typedef std::vector<VectorType> //D: container defined by template argument
      vector_container;

      value_container m_ritz_values; //D: eigenvalue container
      vector_container m_ritz_vectors; //D: eigenvector container
  };
  ...
  namespace detail
  { //D: everything here has this namespace scope
    template <typename VectorType> //D: template argument
    void make_results(unsigned nconv, unsigned n, 
                     typename results<VectorType>::real_type *z, //D:pointer to results struct defined type real_type
                     std::complex<typename results<VectorType>::real_type> *d,//D: complex of (real_type) 
                     results<VectorType> &my_results)//D: pass results by reference
    {
      ...
      typedef std::complex<real_type> complex_type; //D: again with the typedefs

      unsigned i = 0; //D: positive int

      my_results.m_ritz_values.clear(); //D: empties vector belonging to struct
      my_results.m_ritz_vectors.clear();

      while (i < nconv) //D: convergence number nconv
      {
        if (imag(d[i]) != 0) //D: if d has imaginary part
        {
          // complex-conjugate pair
          if (i + 1 >= nconv) //D: throw an error if too many ritz vectors returned
            throw std::runtime_error("arpack: complex pair split up");

          my_results.m_ritz_values.push_back(d[i]); //D: add eval
          my_results.m_ritz_values.push_back(d[i+1]);//D: add eval, and conjugate?

          VectorType ritz_vector(n);
          // un-conjugate ritz vector
          for (unsigned j = 0; j < n; j++)
            ritz_vector[j] = complex_type(z[i*n + j], z[(i+1)*n +j]);
          my_results.m_ritz_vectors.push_back(ritz_vector); //D: add vector

          VectorType ritz_vector_2(n);
          // conjugate ritz vector
          for (unsigned j = 0; j < n; j++) //D: add conjugate vector
        ...
        else
        {
          ...
          VectorType ritz_vector(n); //D: cycles through as before but not in pairs
          for (unsigned j = 0; j < n; j++)
    ...
    template <typename VectorType> //D: make_results for complex vector type passed in
    void make_results(unsigned nconv, unsigned n, 
  ...
  void perform_reverse_communication(
      const MatrixOrOperator &op, //D: passed in as template argument (see matrix_operator)
      const MatrixOrOperator *m, //D: pointer to this template
      arpack_mode mode,
      std::complex<typename boost::numeric::bindings::traits::type_traits<
        typename MatrixOrOperator::operand_type::value_type
      >::real_type> spectral_shift, //D: a double or float
      int number_of_eigenvalues,
      int number_of_arnoldi_vectors,
      results<ResultsVectorType> &res, //D: pass in a results vector type into this struct
      const IterationVectorType &starting_vector, //D: pass in an iterator starting value
      which_eigenvalues which_e = LARGEST_MAGNITUDE, //D: default arguments
      typename boost::numeric::bindings::traits::type_traits<
        typename MatrixOrOperator::operand_type::value_type 
      >::real_type tolerance = 1e-8, //D: default  tolerance of values
      ...
      )
  {
    ...
    int ido = 0;
    char bmat = m == 0 ? 'I' : 'G';//D: ==0 means null pointer (nullptr in c++11); ternary operator sets standard or general eigenproblem
    int n = op.size1();

    if ((unsigned) n != op.size2()) //D: row == col of op
      throw std::runtime_error("arpack: matrix sizes don't match.");
    if ((m != 0) && ((unsigned) n != m->size1() || (unsigned) n != m->size2()))//D: if m!0 and  not same as n
      throw std::runtime_error("arpack: matrix sizes don't match.");

    char *which = const_cast<char*>(detail::map_which_to_string(which_e)); //D: map enum to string

    boost::scoped_array<value_type> residual(new value_type[n]);
    for (int i = 0; i < n; i++)
      residual[i] = starting_vector[i]; //D: copies starting vector to residual

    boost::scoped_array<value_type> v(new value_type[number_of_arnoldi_vectors * n]); //v is "safe" array
    int ldv = n; //D: set ldv = size of matrix
    ...
    int ipntr[14];//array size 14

    boost::scoped_array<value_type> workd(new value_type[3*n]);// workd 3* size of matrix
    int lworkl; //length of wordkl
    lworkl = 3 * number_of_arnoldi_vectors * number_of_arnoldi_vectors 
      + 6 * number_of_arnoldi_vectors; //3* num arnoldi **2+6*arnoldi
    ...
    do
    {
      detail::naupd( //D: actually a disguised macro 
          &ido,
          &bmat,
          &n,
          which,
      ...
      switch (info) //D: just gives info if naupd fails
      {
        case 0:
          break;
        case 1:
      ...
      {
        value_type *x = workd.get() + ipntr[1-1] - 1; //gets x from workd()
